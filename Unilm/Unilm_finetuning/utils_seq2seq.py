...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def read_data(self, line, tokenizer=None):
        sample = line.strip().split('\001')
        src_tk = sample[0]
        tgt_tk = sample[1]
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __init__(self, max_pred, mask_prob, vocab_words, indexer, max_len=512, skipgram_prb=0, skipgram_size=0,
        ...
        self._tril_matrix = torch.tril(torch.ones(
            (max_len, max_len), dtype=torch.long))  # 下三角
    ...
    def __call__(self, instance):
        ...
        segment_ids = [4] * (len(tokens_a) + 2) + [5] * (len(tokens_b) + 1)  # seq2seq语言模型的segment
        ...
        masked_tokens = [tokens[pos] for pos in masked_pos]     # mask token
        for pos in masked_pos:
            if rand() < 0.8:  # 80%
                tokens[pos] = '[MASK]'
            elif rand() < 0.5:  # 10%
        ...
        input_mask = torch.zeros(self.max_len, self.max_len, dtype=torch.long)  # 构造self-attention masks矩阵(上三角)
        input_mask[:, :len(tokens_a) + 2].fill_(1)
        second_st, second_end = len(
            tokens_a) + 2, len(tokens_a) + len(tokens_b) + 3
        input_mask[second_st:second_end, second_st:second_end].copy_(
            self._tril_matrix[:second_end - second_st, :second_end - second_st])    # 下三角=1
