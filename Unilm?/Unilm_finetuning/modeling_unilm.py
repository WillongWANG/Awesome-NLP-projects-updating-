...
from transformers.models.bert.modeling_bert import load_tf_weights_in_bert, BertPooler, BertIntermediate, BertOutput, \
    BertPredictionHeadTransform, BertSelfOutput, BertLMPredictionHead, BertOnlyMLMHead, BertOnlyMLMHead, BertEmbeddings, \
    BertOnlyNSPHead

from configuration_unilm import UnilmConfig

# from transformers import load_tf_weights_in_bert, BertPooler, BertIntermediate, BertOutput, \
#     BertPredictionHeadTransform, BertSelfOutput, BertLMPredictionHead, BertOnlyMLMHead, BertOnlyMLMHead, BertEmbeddings, \
#     BertOnlyNSPHead
...
class UnilmModelIncr(UnilmModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, output_all_encoded_layers=True,
        ...
        embedding_output = self.embeddings(
            input_ids, token_type_ids, position_ids)  # （b_s,len,hidden_size）
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers,
                                      prev_embedding=prev_embedding,
                                      prev_encoded_layers=prev_encoded_layers)  # layer_size * （b_s,len,hidden_size）
        sequence_output = encoded_layers[-1]  # 取最后一层
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, masked_pos=None,
        ...
        sequence_output, __ = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)  # [bs, max_len, hidden_dim]
        ...
        sequence_output_masked = gather_seq_out_by_pos(
            sequence_output, masked_pos)  # 取出来mask的位置的隐向量
        prediction_scores_masked = self.cls(sequence_output_masked)

        # 关于label_smoothing,标签平滑(label-smoothing)主要用于防止过拟合，增强模型的泛化能力。在one-hot的基础上，
        # 添加一个平滑系数ε，使得最大预测与其它类别平均值之间差距的经验分布更加平滑。
        if self.crit_mask_lm_smoothed:
            masked_lm_loss = self.crit_mask_lm_smoothed(
                F.log_softmax(prediction_scores_masked.float(), dim=-1), masked_lm_labels)
        else:
            masked_lm_loss = self.crit_mask_lm(
                prediction_scores_masked.transpose(1, 2).float(), masked_lm_labels)
        masked_lm_loss = loss_mask_and_normalize(
            masked_lm_loss.float(), masked_weights)  # norm loss
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    def __init__(self, config, mask_word_id=0,
        ...
        self.bert = UnilmModelIncr(config)  # encoder
        self.cls = BertOnlyMLMHead(config)
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction='none')
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask):
        ...
        while next_pos < output_length:
            ...
            start_pos = next_pos - curr_length  # 当前编码位置起点
            x_input_ids = torch.cat((curr_ids, mask_ids), dim=1)  # 第一部分序列+ [EOS]
            ...
            new_embedding, new_encoded_layers, _ = \
                self.bert(x_input_ids, curr_token_type_ids, curr_position_ids, curr_attention_mask,
                          output_all_encoded_layers=True, prev_embedding=prev_embedding,
                          prev_encoded_layers=prev_encoded_layers)  # new_encoded_layers:每一层的输出隐向量

            last_hidden = new_encoded_layers[-1][:, -1:, :]  # （b_s,1,hidden_size）, 取每个sample的最后一个token的hidden来进行后面计算
            prediction_scores = self.cls(last_hidden)
            log_scores = torch.nn.functional.log_softmax(
                prediction_scores, dim=-1)  # (bs, 1, vocab_size)
            if forbid_word_mask is not None:
                log_scores += (forbid_word_mask * -10000.0)
            if self.min_len and (next_pos - input_length + 1 <= self.min_len):
                log_scores[:, :, self.eos_id].fill_(-10000.0)
            kk_scores, kk_ids = torch.topk(log_scores, k=K)  # 取出来topk的概率和对应的下标
            if len(total_scores) == 0:
                k_ids = torch.reshape(kk_ids, [batch_size, K])
                back_ptrs = torch.zeros(batch_size, K, dtype=torch.int64)
                ...
                back_ptrs = back_ptrs.to(device=back_ptrs.get_device(), dtype=torch.int64)  # float -> int64
                kk_ids = torch.reshape(kk_ids, [batch_size, K * K])
                k_ids = torch.gather(kk_ids, 1, k_ids)
            step_back_ptrs.append(back_ptrs)  # 指向前面的下标
            step_ids.append(k_ids)  # 记下当前step top_k的token id
            beam_masks.append(torch.eq(k_ids, self.eos_id).float())
            total_scores.append(k_scores)  # 记下当前step top_k的 token score
        ...
        total_scores = [x.tolist() for x in
                        total_scores]  # (batch_size, input_length, K)， batch中所有sample每个时刻topk的score，32×27×5
        step_ids = [x.tolist() for x in step_ids]
        step_back_ptrs = [x.tolist() for x in step_back_ptrs]
        traces = {'pred_seq': [], 'scores': [], 'wids': [], 'ptrs': [], 'top_pred_seq': []}
        for b in range(batch_size):
            scores = [x[b] for x in total_scores]  # （b_s,K）,当前step top_k的 token score
            wids_list = [x[b] for x in step_ids]  # 当前时刻topk的token id
            ptrs = [x[b] for x in step_back_ptrs]  # 前向指针
            ...
            for fid in range(last_frame_id + 1):
                for i, wid in enumerate(wids_list[fid]):
                    if wid == self.eos_id or fid == last_frame_id:  # 遇到终止标志，记下序列
                        s = scores[fid][i]
                        ...
                        if s > max_score:
                            ...
                            frame_id = fid
                            pos_in_frame = i
                        track_all_frame.append((s, fid, i))
            # 取最佳编码序列
            track_all_frame.sort(key=lambda x: x[0], reverse=True)

            def _track_seq(frame_id, pos_in_frame):
                if frame_id == -1:
                    return [0]
                else:
            ...
            top_pred_seq = []
            for _track in track_all_frame[:K]:
                t_frame_id = _track[1]
                t_pos_in_frame = _track[2]
                t_seq = _track_seq(t_frame_id, t_pos_in_frame)
                top_pred_seq.append(t_seq)
            traces['top_pred_seq'].append(top_pred_seq)  # 每个source的topk完整序列token_id
            traces['pred_seq'].append(_track_seq(frame_id, pos_in_frame))   # 每个source的最终预测序列token_id
        ...
        for k in ('pred_seq', 'scores', 'wids', 'ptrs'):  # 结果padding
            ts_list = traces[k]
